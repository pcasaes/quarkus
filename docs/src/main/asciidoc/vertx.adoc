...
You will also have to explicitly configure Vertx to use the native transport. In `application.properties` add:
...
if the connection to the server is established from the same host. This can happen
...
----
quarkus.http.domain-socket=/var/run/quarkus-http.socket
----

By itself this will not disable the tcp socket which by default will open on
`0.0.0.0:8080`. You can disable it by leaving the following environment property empty:
